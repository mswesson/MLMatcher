...
# Транслитерация кириллица→латиница для приведения брендов к общему алфавиту.
_CYR_TO_LAT = {
...
def translit(text: str) -> str:
    ...
    Нужна, чтобы символьные метрики сопоставляли бренды в разных алфавитах:
    «Бельведер» → «belveder», «Belweder» → «belweder» (различие в 1 символ).
...
def levenshtein_token_sort_similarity(s1: str, s2: str) -> float:
    ...
    перестаёт влиять: «парацетамол 500 озон» и «озон парацетамол 500» совпадают.
    """
...
def number_match(s1: str, s2: str) -> float:
    ...
    как мультимножество. «500мг 10шт» и «№10 500 мг» совпадают ([500, 10] == [10, 500]),
...
def number_similarity(s1: str, s2: str) -> float:
    ...
    мелкий «шум формата» (4.5 vs 4 → 0.89) оценивается высоко, а реальная разница
    дозировки (500 vs 250 → 0.5) — низко. Каждое число жадно сопоставляется с
...
def tfidf_cosine_corpus(s1: str, s2: str, vectorizer: TfidfVectorizer) -> float:
    ...
    наборе товаров, поэтому фича несёт реальный сигнал.
    """
    if not s1.strip() or not s2.strip():
        return 0.0
